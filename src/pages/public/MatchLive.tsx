    ...
    <div className="min-h-screen bg-[#0b1120] text-slate-100">
      ...
      <header className="bg-[#0f1a2e] border-b border-slate-700/50 px-4 py-3">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          <Link to="/jogos" className="text-xs text-slate-500 hover:text-slate-300 transition-colors">Ver todos</Link>
        </div>
      </header>

      {/* Category + Phase banner */}
      {match.category && (
        <div className="bg-[#0f1a2e] border-b border-slate-700/50 px-4 py-3 text-center">
          <div className="max-w-2xl mx-auto">
            <p className="text-lg font-extrabold text-amber-400 uppercase tracking-widest">{match.category.name}</p>
            <p className="text-xs text-slate-400 mt-0.5 uppercase tracking-wider">{phaseLabel(match.phase)}</p>
          </div>
        </div>
      )}
